<template>
    ...
    <div class="wrapper">
        <section class="classic">
            ...
            <div class="current">
                <div class="weather">
                    <div class="icon" ref="CurrentIcon"></div>
                    <div class="status">
                        <h1>{{ data.list[0].main.temp }}<span>°C</span></h1>
                        <span ref="CurrentDescription">{{ makeDescription(data.list[0].weather[0].description) }}</span>
                    </div>
                </div>
                <div class="clock"><span ref="HTMLClock"><span></span></span></div>
            </div>
            <div class="continuous">
                <div class="box next-hour">
                    <div class="icon" ref="ContinuousIcon"></div>
                    <div class="status">
                        <h1>3 Saat Sonra</h1>
                        <h3>{{ data.list[1].main.temp }}<span>°C</span></h3>
                        <span>{{ makeDescription(data.list[1].weather[0].description) }}</span>
                    </div>
                </div>
...
const HTMLClock = ref<HTMLSpanElement | null>(null);
const CurrentIcon = ref<HTMLDivElement | null>(null);
const ContinuousIcon = ref<HTMLDivElement | null>(null);
const City = ref<string>("Ankara");
...
const renew = async () => {
    ...
    CurrentIcon.value!.style.backgroundImage = `url('/weather/${makeIcon(data.value.list[0].weather[0].icon)}.svg')`;
    ContinuousIcon.value!.style.backgroundImage = `url('/weather/${makeIcon(data.value.list[1].weather[0].icon)}.svg')`;
...
onMounted(async () => {
    if (!HTMLClock.value || !CurrentIcon.value || !ContinuousIcon.value) return;

    renew();

    setInterval(() => {
        let date = new Date();
        HTMLClock.value!.innerHTML = `${date.getHours()}:${date.getMinutes()}<span>:${date.getSeconds()}</span>`;
    }, 1000)
})
</script>
...
body {
    ...
    background: linear-gradient(0deg, rgba(26, 32, 41, 1) 0%, rgba(49, 96, 146, 1) 100%);
}
